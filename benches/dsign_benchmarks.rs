...
fn ed25519_keygen(c: &mut Criterion) {
    ...
    group.bench_function("gen_key", |b| {
        b.iter(|| {
            black_box(Ed25519::gen_key(black_box(&seed)))
        });
    });
...
fn ed25519_derive_vk(c: &mut Criterion) {
    ...
    group.bench_function("derive_verification_key", |b| {
        b.iter(|| {
            black_box(Ed25519::derive_verification_key(black_box(&sk)))
        });
    });
...
fn ed25519_verify(c: &mut Criterion) {
    ...
    for size in [32, 64, 256, 1024, 4096].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("verify", size), size, |b, _| {
            b.iter(|| {
                black_box(Ed25519::verify(black_box(&vk), black_box(&message), black_box(&signature))).unwrap()
            });
        });
    }
...
fn secp256k1_benchmarks(c: &mut Criterion) {
    ...
    // ECDSA benchmarks
    {
        ...
        group.bench_function("sign", |b| {
            b.iter(|| {
                black_box(Secp256k1Ecdsa::sign(black_box(&sk), black_box(message)))
            });
        });
        ...
        group.bench_function("verify", |b| {
            b.iter(|| {
                black_box(Secp256k1Ecdsa::verify(black_box(&vk), black_box(message), black_box(&signature))).unwrap()
            });
        });
