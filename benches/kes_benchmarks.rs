...
fn kes_sum2_keygen(c: &mut Criterion) {
    ...
    group.bench_function("gen_key_from_seed", |b| {
        b.iter(|| {
            black_box(Sum2Kes::gen_key_kes_from_seed_bytes(black_box(&seed))).unwrap()
        });
    });
...
fn kes_sum2_derive_vk(c: &mut Criterion) {
    ...
    group.bench_function("derive_verification_key", |b| {
        b.iter(|| {
            black_box(Sum2Kes::derive_verification_key(black_box(&sk))).unwrap()
        });
    });
...
fn kes_sum2_sign(c: &mut Criterion) {
    ...
    for size in [32, 64, 256, 1024, 4096].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("sign", size), size, |b, _| {
            b.iter(|| {
                black_box(Sum2Kes::sign_kes(black_box(&()), black_box(0), black_box(&message), black_box(&sk))).unwrap()
            });
        });
    }
...
fn kes_sum2_update(c: &mut Criterion) {
    ...
    group.bench_function("update_kes", |b| {
        b.iter_batched(
            ...
            |sk| black_box(Sum2Kes::update_kes(black_box(&()), black_box(sk), black_box(0))).unwrap(),
...
fn kes_sum6_keygen(c: &mut Criterion) {
    ...
    group.bench_function("gen_key_from_seed", |b| {
        b.iter(|| {
            black_box(Sum6Kes::gen_key_kes_from_seed_bytes(black_box(&seed))).unwrap()
        });
    });
...
fn kes_sum6_sign(c: &mut Criterion) {
    ...
    group.bench_function("sign", |b| {
        b.iter(|| {
            black_box(Sum6Kes::sign_kes(black_box(&()), black_box(0), black_box(message), black_box(&sk))).unwrap()
        });
    });
...
fn kes_sum6_update(c: &mut Criterion) {
    ...
    group.bench_function("update_kes", |b| {
        b.iter_batched(
            ...
            |sk| black_box(Sum6Kes::update_kes(black_box(&()), black_box(sk), black_box(0))).unwrap(),
            criterion::BatchSize::SmallInput,
        );
    });
